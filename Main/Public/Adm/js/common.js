...
//绑定需要浮动的表头
$(function() {
	$('table').addClass('ltable')
	$('.ibox-title').css('background-color', 'white')
	$('.ibox-title').css('border-color', 'white')
	$('.btn').css('color', 'white')
...
function checktxt(obj, txtId) {
	...
	$("#" + txtId)
			.html(
					"您已输入<b>" + txtCount + "</b>个字符，将以<b>" + smsLength
							+ "</b>条短信扣取费用。");
}
// 四舍五入函数
...
// 执行回传函数
function ExePostBack(objId, objmsg, url) {
				...
				.Validform(
						{
							tiptype : function(msg, o, cssctl) {
								...
								if (!o.obj.is("form")) {
									// 定位到相应的Tab页面
									if (o.obj.is(o.curform
											.find(".Validform_error:first"))) {
										var tabobj = o.obj
												.parents(".tab-content"); // 显示当前的选项
										var tabindex = $(".tab-content").index(
												tabobj); // 显示当前选项索引
										if (!$(".content-tab ul li").eq(
												tabindex).children("a")
												.hasClass("selected")) {
											$(".content-tab ul li a")
													.removeClass("selected");
											$(".content-tab ul li")
													.eq(tabindex).children("a")
													.addClass("selected");
											$(".tab-content").hide();
											tabobj.show();
										}
									...
									// 页面上不存在提示信息的标签时，自动创建;
									if (o.obj.parents("dd").find(
											".Validform_checktip").length == 0) {
										o.obj
												.parents("dd")
												.append(
														"<span class='Validform_checktip' />");
										o.obj.parents("dd").next().find(
												".Validform_checktip").remove();
									}
									var objtip = o.obj.parents("dd").find(
											".Validform_checktip");
...
$.fn.ruleSingleCheckbox = function() {
	var singleCheckbox = function(parentObj) {
		...
		// 添加元素及样式
		var newObj = $(
				'<a href="javascript:;">' + '<i class="off">否</i>'
						+ '<i class="on">是</i>' + '</a>').prependTo(parentObj);
...
$.fn.ruleMultiCheckbox = function() {
	var multiCheckbox = function(parentObj) {
		...
		parentObj.find(":checkbox").each(
				function() {
					var indexNum = parentObj.find(":checkbox").index(this); // 当前索引
					var newObj = $(
							'<a href="javascript:;">'
									+ parentObj.find('label').eq(indexNum)
											.text() + '</a>').appendTo(divObj); // 查找对应Label创建选项
					if ($(this).prop("checked") == true) {
					...
					$(newObj).click(
							function() {
								...
								parentObj.find(':checkbox').eq(indexNum)
										.trigger("click"); // 触发对应的checkbox的click事件
...
$.fn.ruleMultiPorp = function() {
	var multiPorp = function(parentObj) {
		...
		parentObj
				...
				.each(
						function() {
							var indexNum = parentObj.find(":checkbox").index(
									this); // 当前索引
							var liObj = $('<li></li>').appendTo(divObj)
							var newObj = $(
									'<a href="javascript:;">'
											+ parentObj.find('label').eq(
													indexNum).text()
											+ '</a><i class="iconfont icon-check-mark"></i>')
									.appendTo(liObj); // 查找对应Label创建选项
							if ($(this).prop("checked") == true) {
							...
							$(newObj).click(
									function() {
										...
										parentObj.find(':checkbox')
												.eq(indexNum).trigger("click"); // 触发对应的checkbox的click事件
										// alert(parentObj.find(':checkbox').eq(indexNum).prop("checked"));
									});
						});
	};
...
$.fn.ruleMultiRadio = function() {
	var multiRadio = function(parentObj) {
		...
		parentObj.find('input[type="radio"]').each(
				function() {
					var indexNum = parentObj.find('input[type="radio"]').index(
							this); // 当前索引
					var newObj = $(
							'<a href="javascript:;">'
									+ parentObj.find('label').eq(indexNum)
					...
					$(newObj).click(
							function() {
								...
								$(this).addClass("selected");
								parentObj.find('input[type="radio"]').prop(
										"checked", false);
								parentObj.find('input[type="radio"]').eq(
										indexNum).prop("checked", true);
								parentObj.find('input[type="radio"]').eq(
										indexNum).trigger("click"); // 触发对应的radio的click事件
...
$.fn.ruleSingleSelect = function() {
	var singleSelect = function(parentObj) {
		...
		// 创建元素
		var titObj = $(
				'<a class="select-tit" href="javascript:;"><span></span><i class="iconfont icon-arrow-down"></i></a>')
				.appendTo(divObj);
		var itemObj = $('<div class="select-items" style="z-index: 10000000;" ><ul></ul></div>').appendTo(
				divObj);
		var selectObj = parentObj.find("select").eq(0); // 取得select对象
		// 遍历option选项
		selectObj.find("option").each(
				function(i) {
					var indexNum = selectObj.find("option").index(this); // 当前索引
					var liObj = $('<li>' + $(this).text() + '</li>').appendTo(
							itemObj.find("ul")); // 创建LI
					...
					liObj.click(function() {
						...
						selectObj.find("option").eq(indexNum).prop("selected",
								true); // 赋值给对应的option
		...
		// itemObj.children("ul").css({ "max-height": $(document).height() -
		// titObj.offset().top - 62 });
		...
		titObj.click(function(e) {
			...
			if (itemObj.is(":hidden")) {
				...
				if (parentObj.parents('.tab-content').length > 0) {
					...
					var itemBttomVal = tabObj.innerHeight()
							- itemObj.offset().top + tabObj.offset().top - 12;
					if (itemBttomVal < itemObj.height()) {
						var itemTopVal = tabObj.innerHeight() - itemBttomVal
								- 61;
