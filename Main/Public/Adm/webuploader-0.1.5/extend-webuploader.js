jQuery(function(){
        ...
        if( file.src == "client"){
            ...
            file.on('statuschange', function(cur, prev){
                ...
                if( cur == 'error' || cur == 'invalid'){
                	...
                	$li
					.append('<div class="file-panel"><span class="cancel "></span></div>');
                    $li.append('<span class="success cancel"></span>');
                    $li.on('mouseenter', function() {
						$(this).find('.file-panel').stop()
								.animate({
									height : 30
								});
					});
        	...
        	$li.append('<input type="hidden" name="img[]" value="'+file.path+'" >')
	...
	function cancel(obj, file) {
		var index = $(obj).index(), deg;
    ...
    uploader.on('uploadSuccess', function(file,ret){
        ...
        $('#' + file.id ).append('<input type="hidden" name="img[]" value="'+ret.path+'" >')
