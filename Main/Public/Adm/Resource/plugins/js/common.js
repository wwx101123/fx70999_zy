...
//执行回传函数
function ExePostBack(objId, objmsg, sendUrl) {
    ...
    var checkValue='';
    $('.checkall').find("#chkId").each(function() { 
    	if($(this).prop("checked") == true) {
    		
    		checkValue=checkValue+$(this).attr('goods_id')+',';
        ...
        ok: function () {
        	...
        	$.ajax({
				...
				success: function(data, textStatus) {
					if (data.status == 1){
						var tipdialog = parent.dialog({content:data.msg}).show();
						setTimeout(function () {
