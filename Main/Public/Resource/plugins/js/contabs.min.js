...
function refresh() {
    $(".J_menuTab").each(function () {
        if ($(this).hasClass("active")) {
            thisTab = $(this).data("id");
        }
    });
    $(".J_iframe").each(function () {
        if ($(this).data("id")==thisTab) {
...
/*切换主题*/
function changeThem(skin,obj){
    var thisSkin = $("#admin_skin").text();
    if(thisSkin!=skin){
        $.get('main.php?action=skin&type=admin&skin='+skin);
        $(".check_skin").not($(obj).children("b")).html(" ");
        $(obj).children("b").html('<i class="fa fa-check"></i>');
        $("#admin_skin").text(skin);
    }
