...
class Validate
{
     ...
     * @return mixed
     */
     function checkItem($field, $value, $rules, $data, $title = '', $msg = [])
    {
     ...
     * @return array
     */
    public function getValidateType($key, $rule)
    {
        // 判断验证类型
        if (!is_numeric($key)) {
            return [$key, $rule, $key];
     ...
     * @return bool
     */
    public function activeUrl($value, $rule)
    {
     ...
     * @return string
     */
    public function getRuleMsg($attribute, $title, $type, $rule)
    {
        
        $request= import("@.ORG.Lang");
        $Lang = new Lang();
