...
class TreeAction extends CommonAction
{
    ...
    public function Tree()
    {
        ...
        $rows['0'] .= "tree.icons['Center']  = 'center.gif';" . chr(13) . chr(10); // 服务中心成员
    ...
    private function _MakeTree($ID, $FatherId, $IsZs, $N, $j, &$rows)
    {
        ...
        if ($N <= 100) {
            ...
            $where['re_id'] = $ID;
            $rs = $fck->where($where)
                ->order('is_pay desc,pdt asc,id asc')
                ->select();
            foreach ($rs as $rss) {
                $j = $j + 1;
                $uUser = $rss['user_id'];
    ...
    public function Tree1()
    {
        ...
        $out_rs = $fck->where($out_where)
            ->field('id,user_id,is_pay,is_agent,u_level')
            ->order('id asc')
            ->limit(9)
            ->select();
        // dump($fck->getLastSql());
        ...
        for ($u = $i + 1; $u <= 10; $u ++) {
            ...
            if ($u < 10) {
                $Treex .= "<tr align='center'><td><img src='" . $tps . "bottom.gif' height='" . $lhe . "'></td></tr>";
            }
        }
        $Treex .= "</table>";
    ...
    public function Tree2()
    {
        ...
        if (! empty($UserID)) {
            ...
            // $where = "user_id='". $UserID ."' ";
            $field = 'id,is_boss,is_money,extra_money';
            $rs = $fck->where($where)
                ->field($field)
                ->find();
            if ($rs == false) {
        ...
        if (! $rs) {
            ...
            $count = M('duipeng_num')->where('  user_id=' . $ID)->

            sum('sum_level');
            if ($count == NULL) {
                $count = 0;
            }
            $count_l = M('duipeng_num')->where('  user_id=' . $ID)->

            sum('sum_level_l');
            if ($count_l == NULL) {
                $count_l = 0;
            }
            $count_r = M('duipeng_num')->where('  user_id=' . $ID)->
        ...
        for ($i = 1; $i <= $Nums; $i ++) {
            ...
            $TreeArray[$i] = "<table border='0' cellpadding='0' cellspacing='1' class='tu_box'><tr><td class='tu_ko'> 空位" . $kong . " </td></tr></table>";
        }
        ...
        $MyYJ = "</td></tr>";
        // $MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#' title='用户账号：".$UserID."|手机：".$user_tel."|QQ：".$qq."'>联系方式</a></td></tr>";
        $MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#'>$HYJJ[$uLevel]</a></td></tr>";
        ...
        $MyYJ .= "<tr><td class='tu_l' $bj>$benqiL</td><td class='tu_z' $bj>碰</td><td class='tu_r' $bj>$benqiR</td></tr>";
        $MyYJ .= "</table>";
        ...
        if ($_POST['is_mobile'] == 1) {
            ...
            $Str4C1 = "<a href='javascript:void(0)' class='user'   RID =" . $myid . " ";
        }
        $Str4C4 = "</td></tr></table>";
        if ($isPay > 0) {
            $i = pow(2, $uLev);
            if ($_POST['is_mobile'] == 1) {
                $F = $fck->where('id=' . $Rid)
                    ->field('user_id')
                    ->find();
                $TreeArray['1'] = $Str4C0 . $Str4C1 . "FID=" . $F['user_id'] . " TPL=0>点击</a>" . $Str4C4;
                $TreeArray[$i] = $Str4C0 . $Str4C1 . "FID=" . $F['user_id'] . " TPL=1>点击</a>" . $Str4C4;
            } else {
        ...
        if ($_POST['is_mobile'] == 1) {
            ...
            $wop = str_replace('__PUBLIC__/', $_POST['IP'] . __ROOT__ . '/Public/', $wop);
            $data = array();
            $data['msg'] = '获取成功';
            $data['status'] = 1;
            $data['wop'] = $wop;
    ...
    private function Tree2_MtKass($FatherID, $iL, $pLevel, $uLev, $Str4C0, $Str4C1, $Str4C4, &$TreeArray, $Nums)
    {
        ...
        if (! empty($FatherID)) {
            ...
            foreach ($rs as $rss) {
                ...
                $count = M('duipeng_num')->where('  user_id=' . $rss['id'])->

                sum('sum_level');
                if ($count == NULL) {
                    $count = 0;
                }
                $count_l = M('duipeng_num')->where('  user_id=' . $rss['id'])->

                sum('sum_level_l');
                if ($count_l == NULL) {
                    $count_l = 0;
                }
                $count_r = M('duipeng_num')->where('  user_id=' . $rss['id'])->
                ...
                $Rid = $rss['id'];
                $uUserID = $rss['user_name'];
                if ($_POST['is_mobile'] != 1) {
                    if ($rss['is_money'] == 0) {
                        $uUserID = $uUserID . '【空单】';
                        if ($rss['extra_money'] > 0) {
                            $uUserID = $uUserID . $rss['extra_money'];
                        }
                    }
                }
                ...
                $MyYJ = "</td></tr>";
                // $MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#' title='用户账号：".$uUserID."|手机：".$uuser_tel."|QQ：".$uqq."'>联系方式</a></td></tr>";
                $MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#'>$HYJJ[$Leve]</a></td></tr>";
                ...
                $MyYJ .= "<tr><td class='tu_l' $bj>$benqiLo</td><td class='tu_z' $bj>碰</td><td class='tu_r' $bj>$benqiRo</td></tr>";
                $MyYJ .= "</table>";
                
                // $Str = $StTab."<a href='". __URL__ ."/PuTao/ID/". $Rid ."'>用户账号：". $uUserID ."</a>". $MyYJ;
                
                $Str = $StTab . "<a href='" . __URL__ . "/Tree2/ID/" . $Rid . "'>" . $uUserID . "</a>" . $MyYJ;
                if ($_POST['is_mobile'] == 1) {
                    
                    $Str = $StTab . "<a onclick='get_tree2(" . $Rid . ")' href='javascript:void(0)'>" . $uUserID . "</a>" . $MyYJ;
                }
                ...
                if ($uisPay > 0) {
                    if ($Yo <= $Nums + 1 && $i > 0) {
                        ...
                        if ($_POST['is_mobile'] == 1) {
                            ...
                            $TreeArray[$Uo] = $Str4C0 . $Str4C1 . " TPL=0" . $Str4C2 . ' ' . $Str4C4;
                            $TreeArray[$Yo] = $Str4C0 . $Str4C1 . " TPL=1" . $Str4C2 . '   ' . $Str4C4;
                        }
                    }
                } else {
                    // if ($Yo<=$Nums+1 && $i>0){
    ...
    public function todayindan($uid = 0, &$danL = 0, &$danR = 0)
    {
        ...
        if ($rs) {
            $rs_l = $fck->where('father_id=' . $uid . ' and treeplace=0')
                ->field('id')
                ->find();
            if ($rs_l) {
                $lid = $rs_l['id'];
            ...
            $rs_r = $fck->where('father_id=' . $uid . ' and treeplace=1')
                ->field('id')
                ->find();
            if ($rs_r) {
                $rid = $rs_r['id'];
    ...
    public function Tree3()
    {
        ...
        if (! empty($UserID)) {
            ...
            // exit;
            if ($rs == false) {
                $this->display('tree3none');
                $this->error('您在该网络内没有账户');
                exit();
            } else {
                $UID = $rs['id'];
            }
        ...
        if ($isPay > 0) {
            ...
            $TreeArray['1'] = $Str4C0 . $Str4C1 . $Str4C4;
            $TreeArray[$j] = $Str4C0 . $Str4C1 . $Str4C4;
            $TreeArray[$i] = $Str4C0 . $Str4C1 . $Str4C4;
        } else {
            // $TreeArray['1']=$Str4C0.$Str4C1."0/RID/". $ID ."/FID/".$ID."' target='_self'>点击注册</a>".$Str4C4;
    ...
    public function TreeAjax()
    {
        ...
        $myName = $UserID . "(" . $username . ") [" . getUserLevel($rs['u_level']) . "]";
        $myTabN = "m" . $UID;
        ...
        $trs = $fck->where($rwhere)
            ->order('is_pay desc,pdt asc')
            ->select();
        $zz = 1;
        foreach ($trs as $rss) {
            $rssid = $rss['id'];
            ...
            $HYJJ = '';
            $this->_levelConfirm($HYJJ, 1);
            $z_myName = $rsuserid . "(" . $rss['user_name'] . ") [" . getUserLevel($rss['u_level']) . "]";
        ...
        if (! empty($keywords)) {
            $user = M('user')->where(" member_id like '%" . $keywords . "%'")
                ->order('id asc ')
                ->find();
            if ($user != null) {
                $user_id = $user['id'];
        ...
        // $sql = "call get_recommend_list(1)";
        // $tree = M('fck')->query($sql);
        // $trees = array();
        // foreach ($tree as $k => $v) {
        // $user = M('fck')->where("id=" . $v['id'])->find();
        // $tree[$k]['pId'] = $v['reference_id'];
        // $tree[$k]["user_name"] = $user['user_id'];
        // $tree[$k]["name"] = $user['name'] . "【" . $user['user_id'] . "】";
        // $tree[$k]["open"] = 'true';
        // }
        // $this->assign('zNodes', json_encode($tree));
    ...
    public function ajax_tree_m()
    {
        ...
        foreach ($z_tree as $zzzz) {
            ...
            $zzz_str .= '<p>' . $zzzz[2] . $zzzz[0] . '</p>' . '<table width="100%" border="0" cellspacing="0" cellpadding="0" id="' . $zzzz[1] . '" class="treep2">' . '<tr><td id="' . $zzzz[1] . '_tree">' . $ttt_nnn . '</td>' . '</tr></table>';
        }
    ...
    public function TreeAjaxb()
    {
        ...
        $trs = $fck->where($rwhere)
            ->order('treeplace asc,is_pay desc,pdt asc')
            ->select();
        $zz = 1;
        foreach ($trs as $rss) {
            $rssid = $rss['id'];
    ...
    public function ajax_tree_mb()
    {
        ...
        foreach ($z_tree as $zzzz) {
            ...
            $ttt_nnn = $this->cf_img($ttt_mm, $n_path);
            $zzz_str .= '<p>' . $zzzz[2] . $zzzz[0] . '</p>' . '<table width="100%" border="0" cellspacing="0" cellpadding="0" id="' . $zzzz[1] . '" class="treep2">' . '<tr><td id="' . $zzzz[1] . '_tree">' . $ttt_nnn . '<img src="' . __PUBLIC__ . '/Images/loading1.gif" align="absmiddle"></td>' . '</tr></table>';
        }
        $this->assign('zzz_str', $zzz_str);
        $this->display();
        exit();
    ...
    public function yidongTwo()
    {
        ...
        $fck_rs = $fck->where($where)
            ...
            ->find();
        if (! $fck_rs) {
            $this->error('没有找到该用户账号，请重新输入用户账号！');
            exit();
        } else {
            if ($fck_rs['id'] == 1) {
                $this->error('根节点不能移动，请重新输入用户账号！');
                exit();
            }
        }
        $fck_frs = $fck->where($fwhere)
            ->field($field)
            ->find();
        if (! $fck_frs) {
            $this->error('没有找到移至的用户账号，请重新输入用户账号！');
            exit();
        } else {
            if ($fck_frs['is_pay'] == 0) {
                $this->error('移至的用户账号尚未开通，请重新输入用户账号！');
                exit();
            }
            // 分割成數組進行比較
            $arr = explode(',', $fck_frs['re_path']);
            if (in_array($fck_rs['id'], $arr)) {
                $this->error('移至用户账号在 ' . $sUserID . ' 的团队下面，请重新输入！');
                exit();
            }
        }
        ...
        $vwhere['re_path'] = array(
            'like',
            $fck_rs['re_path'] . $fck_rs['id'] . '%'
        );
        $vo = $fck->where($vwhere)
            ->field($field)
            ->order('re_level asc')
    ...
    public function Tree2_B()
    {
        ...
        $ffck2 = $fck2->where('fck_id=' . $_SESSION[C('USER_AUTH_KEY')])
            ->order('id asc')
            ->select();
        $this->assign('ffck2', $ffck2);
        
        $ffrs = $fck2->where('fck_id=' . $_SESSION[C('USER_AUTH_KEY')])
        ...
        if (! $rs) {
            ...
            $renums = $fck->where('id=' . $rs['fck_id'])->getField('re_nums');
            $reid = $fck->where('id=' . $rs['fck_id'])->getField('re_id');
            $reuserid = $fck->where('id=' . $reid)->getField('user_id');
            $reusername = $fck->where('id=' . $reid)->getField('user_name');
        ...
        $StTab = "<table border='0' cellpadding='0' cellspacing='1' class='tu_box'><tr><td colspan='3' style='background:" . $ji_c[$uLevel] . ";font-weight:bold;'>";
        $MyYJ = "</td></tr>";
        $MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#' title='姓名：" . $UserName . "|推荐人：" . $reuserid . "|推荐人姓名：" . $reusername . "|分享人数：" . $renums . "|注册时间：" . $renums . "'>详情</a></td></tr>";
        $MyYJ .= "</table>";
    ...
    private function Tree2_B_MtKass($FatherID, $iL, $pLevel, $uLev, $Str4C0, $Str4C1, $Str4C4, &$TreeArray, $Nums)
    {
        ...
        if (! empty($FatherID)) {
            ...
            // $where = array();
            $ppp = $uLev + $pLevel;
            $whe = "father_id=" . $FatherID . " and p_level<=" . $ppp . " and treeplace<2";
            $field = '*';
            $rs = $fck2->where($whe)
                ->field($field)
                ->order('treeplace asc')
            ...
            foreach ($rs as $rss) {
                ...
                $Rid = $rss['id'];
                $uUserID = $rss['user_id'];
                $uUserName = $rss['user_name'];
                $uisPay = $rss['is_pay'];
                $upLevel = $rss['p_level'];
                
                $urenums = $fck->where('id=' . $rss['fck_id'])->getField('re_nums');
                $ureid = $fck->where('id=' . $rss['fck_id'])->getField('re_id');
                $ureuserid = $fck->where('id=' . $ureid)->getField('user_id');
                $ureusername = $fck->where('id=' . $ureid)->getField('user_name');
                ...
                $StTab = "<table border='0' cellpadding='0' cellspacing='1' class='tu_box'><tr><td colspan='3' style='background:" . $ji_c[$Leve] . ";font-weight:bold;'>";
                $MyYJ = "</td></tr>";
                $MyYJ .= "<tr><td colspan='3' $bj><a class='title' href='#' title='姓名：" . $uUserName . "|推荐人：" . $ureuserid . "|推荐人姓名：" . $ureusername . "|分享人数：" . $urenums . "'>详情</a></td></tr>";
                $MyYJ .= "</table>";
                
                // $Str = $StTab."<a href='". __URL__ ."/PuTao/ID/". $Rid ."'>用户账号：". $uUserID ."</a>". $MyYJ;
                $Str = $StTab . $uUserID . $MyYJ;
                $Str4C2 = "/FID/" . $Rid . "'>点击注册</a>";
    ...
    public function user_tree()
    {
        ...
        if (! empty($keywords)) {
            $user = M('fck')->where(" user_id like '%" . $keywords . "%'")
                ->order('id asc ')
                ->find();
            if ($user != null) {
                $user_id = $user['id'];
        ...
        $sql = "call get_recomend_list(" . $user_id . ")";
        
        $tree = M('fck')->field(' id,re_id as pId, true  as open,  user_id , re_id ,user_id as name,user_name,0 AS level,get_level')
            ->where(' re_path like "%,' . $user_id . ',%" OR ID='.$user_id)
            ->select();
        ...
        foreach ($tree as $k => $v) {
            ...
            // $HYDJ ='临时会员';
            // } else {
            // $HYDJ = $HYDJList[ $v['get_level'] - 1];
            // }
            
            // $tree[$k]['name'] =$v['user_id'].'【'.$YHDJ.'】';
        }
        $this->assign('zNodes', json_encode($tree));
        $this->assign('keywords', $keywords);
        $this->assign('all_nn', count($tree));
        
        $this->display('user_tree');
    }
}
?>