...
//         }
    ...
    * 微信初始化认证
    **/
    public function wechatInitAuth(){
    ...
    /**
   	...
   	**/
    public function wechatWebAuth($redirct_url = "",$scope = "snsapi_userinfo",$user_id = ""){
    	$redirct_url = $redirct_url === ""?C("wx_webauth_callback_url"):urlencode($redirct_url);
    	$wxurl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=".C("wx_appID")."&user_id=".$user_id."&redirect_uri=".$redirct_url."&response_type=code&scope=".$scope."&state=STATE#wechat_redirect";
    ...
    **/
    ...
    /**
   	...
   	**/
    public function wxGetTokenWithCode($code){
    	if(!isset($code)){
    		return false;
    	}
    ...
    protected function get_user_info() {
        ...
        $url = "https://api.weixin.qq.com/sns/userinfo?access_token=".S("wx_access_token")."&openid=". S('openid')."&lang=zh_CN";
        $Token = $this->curlGetInfo($url);
        file_put_contents("curlGetInfo.txt", $Token, FILE_APPEND);
        $data = json_decode($Token, true);
