	...
	<view :style="{'padding-top':padTop,'padding-bottom':padBottom}" @touchstart="touchstartEvent" @touchmove="touchmoveEvent" @touchend="touchendEvent" @touchcancel="touchendEvent">
		...
		<view v-if="optDown" class="mescroll-downwarp" :class="{'mescroll-downwarp-reset':isDownReset}" :style="{'height': downHight+'px', 'position': 'relative', 'overflow': 'hidden', '-webkit-transition': isDownReset?'height 300ms':''}">
			<view class="downwarp-content">
				<view class="downwarp-progress" :class="{'downwarp-progress-slow':!isDownLoading,'downwarp-progress-fast':isDownLoading}"></view>
			</view>
		</view>
...
<script>
	...
	// 引入全局配置
	import GlobalOption from './mescroll-meitun-option.js';
			...
			let diyOption = {
				...
				down: {
					inOffset(mescroll) {
						...
						vm.isDownLoading = false; // 显示慢的动画
					},
					outOffset(mescroll) {
						// 下拉的距离大于offset那一刻的回调
						vm.isDownReset = false;
						vm.isDownLoading = true; // 显示快的动画
					},
...
<style>
	...
	.mescroll-downwarp .downwarp-progress-slow{
		-webkit-animation: animProgress 2s steps(1,end) infinite;
		animation: animProgress 2s steps(1,end) infinite;
	}
	/*下拉刷新--进度-快*/
	.mescroll-downwarp .downwarp-progress-fast{
		-webkit-animation: animProgress .6s steps(1,end) infinite;
		animation: animProgress .6s steps(1,end) infinite;
	}
	/*下拉刷新--进度-动画*/
	@-webkit-keyframes animProgress {
		0% {background-image: url(http://www.mescroll.com/img/meitun/mescroll-progress1.png)}
		16% {background-image: url(http://www.mescroll.com/img/meitun/mescroll-progress2.png)}
		32% {background-image: url(http://www.mescroll.com/img/meitun/mescroll-progress3.png)}
		48% {background-image: url(http://www.mescroll.com/img/meitun/mescroll-progress4.png)}
		64% {background-image: url(http://www.mescroll.com/img/meitun/mescroll-progress5.png)}
		80% {background-image: url(http://www.mescroll.com/img/meitun/mescroll-progress6.png)}
		100% {background-image: url(http://www.mescroll.com/img/meitun/mescroll-progress1.png)}
	}
