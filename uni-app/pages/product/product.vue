	...
	<view class="container">
		...
		<view class="introduce-section">
			...
			<view class="price-box">
				...
				<text class="price">{{goods.market_price}}</text>
				<text class="m-price">¥{{goods.price}}</text>
				<!-- <text class="coupon-tip">7折</text> -->
			</view>
			<view class="bot-row">
				<text>销量: {{goods.sell_count}}</text>
				<text>库存: {{goods.stock}}</text>
				<text>浏览量: {{goods.click}}</text>
			</view>
		</view>
		...
		<view class="cu-list menu-avatar eva-section"   v-if="seller!=null"    >
			<view class="cu-item b-b">
				...
				<view class="content flex-sub">
					<view class="text-grey" style="color: black;">{{seller.title}}</view>
					<view class="text-gray text-sm flex justify-between">
						商品数量:{{seller.goods_num}}

					</view>

				</view><button style="float: right;width:80px;" class='cu-btn bg-green shadow' @click="navTo(seller.url)">进店逛逛</button>
			</view>
...
高
		...
		<view class="c-list">
			<!-- <view class="c-row b-b" @click="toggleSpec" v-show="false" >
			...
			<view class="c-row b-b" v-show="false">
		...
		<view class="page-bottom">
			...
			<navigator url="/pages/cart/cart" open-type="switchTab" class="p-b-btn">
				<view class='cu-tag badge'>{{cart_count}}</view>
			...
			<view class="action-btn-group">
				...
				<button type="primary" class=" action-btn no-border add-cart-btn" :data-img="goods.cart_img" @click="addShopCar">加入购物车</button>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			detail(userInfo, goods_id) {
				let that = this;
				uni.request({
					url: common.goods_editUrl, //仅为示例，并非真实接口地址。
					data: {
						is_mobile: 1,
						user_id: 0,
						func: 'show',
					...
					success: function(res) {
						if (res.data.status == 1) {
							...
							let carouselList = res.data.data.slider;
				...
				that.specList.forEach(item => {
					for (let cItem of that.specChildList) {
			...
			buy() {
				...
				uni.request({
					url: common.cart_goods_addUrl, //仅为示例，并非真实接口地址。
					data: {
						"actiontype": 'buy',
						"is_mobile": 1,
