	...
	<view>
		...
		<view class="form">
			<view class="username">
				...
				<input placeholder="请输入手机号" v-model="phoneNumber" value="" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<!-- <view class="code">
			...
			<view class="code">
				<input placeholder="请输入推荐人编号" v-model="RID"  placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
	...
	export default {
		...
		methods: {
			...
			doReg(){
				...
				let that=this;
				console.log(common.usersAdd)
				//模板示例把用户注册信息储存在本地，实际使用中请替换为上传服务器。
				setTimeout(()=>{
					uni.request({
						url: common.usersAdd, //仅为示例，并非真实接口地址。
						data: {
							is_mobile: 1,
							user_id: that.phoneNumber,
							pwd: that.passwd,
							RID:that.RID,
							u_level:that.u_level,
							get_level:that.get_level,
							register_type:that.register_type
						},
						method: 'POST',
						header: {
